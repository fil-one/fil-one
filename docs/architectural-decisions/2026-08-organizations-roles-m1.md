...
**Status:** Accepted
**Created:** 2026-08-14
**Implementation:** The PRs stacked above this ADR's own PR (#596) ship it: #597–#610 plus the close-out #617, under stack #611. File paths below name modules as they exist at the top of that stack.
...
**Coordination with the FIL-112 deletion stack** (a stacked-PR series with its own proposed ADR, `2026-08-self-serve-account-deletion.md` on its branch). That stack introduces the codebase's first real role check: an `isOrgAdmin()` helper in `lib/org-membership.ts` that reads the `MEMBER#` row with `ConsistentRead` and compares `role === OrgRole.Admin`, called by both the deletion-request and deletion-confirm handlers. The row that helper reads stops existing when the conversion moves membership into `OrgTable` and deletes the `UserInfoTable` originals — after that, `isOrgAdmin()` returns false for everyone and deletion is denied outright. Sequencing is settled: the FIL-112 stack merges first; this stack rebases onto it and patches the helper, reading the `OrgTable` row with the transition fallback before the conversion runs, then folding into `authorize('org.delete')` when enforcement lands. `isOrgAdmin()` never becomes a second enforcement idiom.
...
In `resolveUserAndOrg`: if the header is absent (curl, old clients), the active org is the identity row's `orgId` — the personal org, preserving today's behavior. If present, the middleware reads `ORG#{header}/MEMBER#{userId}`, the same read that resolves the role, and a missing row is a 403 `NOT_A_MEMBER`. `userInfo.orgId` becomes the active org, so every downstream handler, guard, and key expression stays untouched. Nothing about the token changes; a revoked membership dies on the next request. Because an absent header silently means "personal org", the console always sends the header, and `GET /api/me` echoes the org it actually served.
...
1. **Foundations and scripts**: the enum, the `OrgTable` declaration, write-path changes, `createdBy`, the conversion script, the absent-row fallback, `MeResponse` exposure. The conversion runs here.
...
3. **Audit write path and billing dual-write**, independent of each other, both after enforcement. The billing backfill, flip, and cleanup complete before invitations exist, because the first accepted invitation creates the first real second member.
...
"Which orgs does this user belong to" has no access pattern today. A GSI would be the first in the codebase, and **DynamoDB serves every GSI read eventually consistent — there is no `ConsistentRead` on an index.** This read is on the authorization path: the membership `GetItem` that resolves a caller's role and the `Query` behind `MeResponse.memberships` both set `ConsistentRead` today (`lib/org-membership.ts:144,201`), because an accepted invitation, a role change, or a removal moments earlier must not read as if it never happened, and the `ownerCount` recount in §1 needs the same guarantee to be worth running. An index cannot give it at any scale. Backfill duration is the smaller matter and would be tolerable at today's ~1,000 membership rows; consistency is the property that does not improve as the table grows. The codebase's own answer is the inverse item (`RAGKEYHASH#…/LOOKUP`, documented at `lib/rag-api-keys.ts:16-18`), which rides the same `TransactWriteItems` as the row it mirrors and reads consistently.
