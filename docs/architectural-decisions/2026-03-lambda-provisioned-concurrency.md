...
- **Cost**: ~$82/month for provisioned concurrency in us-east-2 — 7 functions at 512 MB plus 5 functions at 1024 MB, 1 provisioned instance each. This is a fixed cost independent of invocation count. Check AWS Cost Explorer under Lambda > `Provisioned-Concurrency` usage type.
- **Version management**: Every deploy to a PC-enabled stage publishes a new Lambda version. Old versions accumulate. AWS retains them indefinitely by default; periodic cleanup may be needed to avoid hitting the default deployment package storage limit.
