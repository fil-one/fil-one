...
- The FilOne backend aborts any Management API or S3 Gateway request that has not answered within its own deadline: 8 s for calls made on behalf of an interactive request, 25 s for tenant provisioning and key rotation, and 30 s for background jobs. An aborted call fails like any other transport error and is reported as `statusGroup=timeout`. Service Orchestrators should answer well inside those bounds; a `PUT /tenants/{tenantId}` that holds the request open longer than 25 s will be retried on the user's next attempt.
