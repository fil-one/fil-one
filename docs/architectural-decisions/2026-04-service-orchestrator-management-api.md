...
- The FilOne backend aborts any Management API or S3 Gateway request that has not answered within its own deadline: 8 s for calls made on behalf of an interactive request, 20 s for tenant provisioning and key rotation, 8 s for a call that undoes work the request already did (deleting a key whose record did not land, deleting a tenant whose profile write was refused, revoking a rotated key), and 30 s for background jobs. A compensating call gets its own deadline so it still runs when the work it undoes used the whole budget. An aborted call fails like any other transport error. Management API aborts are reported as `statusGroup=timeout` in the API client metrics; S3 Gateway aborts appear only in the calling handler's error log, because the S3 client is not instrumented. Service Orchestrators should answer well inside those bounds; a `PUT /tenants/{tenantId}` that holds the request open longer than 20 s will be retried on the user's next attempt.
