...
- `PUT /tenants/{tenantId}` performs create & setup synchronously and returns only after the tenant is fully operational. The call is idempotent on the client-supplied `tenantId` in the URL path: re-calling with the same identifier returns the existing tenant. The `tenantId` must be a UUID. UUID lets every Service Orchestrator persist the value efficiently and rules out cross-partner collisions by construction. FilOne derives the `tenantId` as a UUIDv5 over `{organisation-id}/{region}`, so it needs no upstream-minted identifier and no pre-commit write: a retry recomputes the same value and lands idempotently on the same tenant. PUT is the natural verb: the client supplies the identifier and the operation is idempotent by definition.

  Amended 2026-09: this originally specified the organisation ID verbatim. A tenant is bound to its region for life and one Service Orchestrator may serve several regions, so an identifier keyed on the organisation alone locks an organisation out of every region after its first. The derivation is frozen — see `packages/backend/src/lib/orchestrator/tenant-id.ts`. Organisations provisioned before the change keep their original identifier, which FilOne stores per region.
