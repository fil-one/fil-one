...
The two paths send different idempotency keys for the create. Tenant setup sends `console-key-v2-${stage}-${tenantId}-${storageUser.id}`, keeping the scoping that gives a re-provisioned org a fresh key and adding the `-v2` segment for the payload change: without it, a tenant whose setup crashed between the create and the `fthTenantId` write would replay the pre-v2 key with the new name and actions and 409 forever. The rotation script cannot send a tenant's original setup key at all, because that key is bound to the payload of that create: replaying it with the v2 name and actions would 409 permanently and no existing tenant could be rotated. It sends `console-key-v2-${tenantId}-${random}`, fresh on every attempt. A key derived from the tenant id alone caused the production failure of 2026-09-01: a re-run had FTH replay the stored 201 of the earlier create and hand back the accessKeyId of a key that same re-run had just deleted. Nothing needs the replay, because names are unique within a tenant: a create FTH already committed comes back as a name conflict rather than a second key.
...
SSM is written only after the new key answers a GET on its id and signs an S3 `ListBuckets`. A 201 from the create is not evidence the key exists: production hit exactly that on 2026-09-01, and the tenant whose verification fails keeps signing with the key it already has.
...
`repair` covers a failure production hit on 2026-09-01. For three tenants FTH answered the create with an accessKeyId, `rotate` wrote those credentials to SSM, and the key does not exist: it is absent from the tenant's listing, a GET on the id returns 404, and it cannot sign an S3 request. `prune` refuses such a tenant, so the v1 key survives and the console keeps working until the Lambda containers holding the pre-rotation credentials recycle.

The v1 secret cannot be recovered, so the repair is a new key. `repair` acts only on a tenant whose SSM-referenced id FTH answers with a 404, and creates `filone-console-v2-fix-<random>` under an idempotency key derived from that name. The random suffix clears whatever name state the failed create left behind at FTH, which is what `rotate` cannot get past for such a tenant: it always asks for the name `filone-console-v2`.
