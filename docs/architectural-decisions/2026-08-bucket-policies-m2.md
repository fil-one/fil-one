# Bucket access by region: IAM policies on Forge, scoped keys on Aurora and FTH

**Status:** Draft (design exploration, awaiting acceptance)
**Created:** 2026-08-26
**Ticket:** FIL-1017 (IAM milestone 2)
**Builds on:** [`2026-08-organizations-roles-m1.md`](./2026-08-organizations-roles-m1.md)
**Supersedes:** #642, #661, #662, #667

## Overview

FIL-1017 requires Owners and Admins to be able to grant members access to a
subset of an organization’s buckets. Members with scoped access should only be
able to see and operate on those buckets.

Forge can support this because we control its management API. Aurora and FTH
cannot currently model per-member bucket access, so access in those regions will
continue to be determined by role. As a result, the console will support two
access models and select the appropriate one based on the region.

Forge will use AWS-style bucket policies. Aurora and FTH will continue using the
scoped access keys they support today, with permissions still bounded by the
member’s role.

No production region currently uses the IAM model, so bucket visibility will not
change in `eu-west-1` or `us-east-1`. Production customers will not see bucket
policies until a Forge region reaches GA. We are therefore choosing to satisfy
FIL-1017 only in IAM-enabled regions.

Access is scoped at the bucket level. Prefix-level access within a bucket is out
of scope for now and can be addressed separately.
...
reissuing any of them, and bucket policies grant bucket-level access to users by
name.
...
**unscoped**: on an IAM region the console writes them onto every bucket's
policy.
...
holder could no longer mint. Every member sees every bucket and the role decides
what they may do. There is no bucket-level scoping of members on either
...
A Forge network moves from scoped keys to IAM once and does not move back.
...
open. Org-wide aggregates also stay org-wide, so a scoped member can still learn
that other buckets exist. Scoping those aggregates would need a per-bucket
breakdown of usage, billing, and dashboard counts, and a scoped member's numbers
would then stop matching the invoice.
...
log. The enforcing system owns the rule it enforces, so there is no second copy
of a policy to drift. The one thing the console pushes into the storage system
is role membership, as the statements it writes for Owners and Admins. Those
need reconciling.
...
**The console grants Owners and Admins every bucket explicitly.** The storage
system knows only statements, so an Owner or Admin reaches every bucket only
because the console put them on every bucket's policy. Every new bucket's policy
names the org's current Owners and Admins, and a promotion or demotion rewrites
the statements on every bucket in the region. The statement the console writes
for an Owner carries every action; the one it writes for an Admin leaves out the
...
The member detail page carries a read-only list of the buckets a member reaches,
so an admin can answer "what does this person have" from the person's page.
...
strong ETag over the stored document, a write carries it back, and a write whose
ETag no longer matches is refused with nothing written. Creating a bucket's
first policy is the same check against no policy existing.
...
roster change is an update to it and there is no separate members-changed event.
One event per action rather than per member, since an edit touching twenty
members would otherwise write twenty events.
...
**A role narrowing.** The admin sees what will happen before confirming.
...
2. Every local precondition, the owner count included, is checked before the
   first vendor call.
3. On scoped-key regions the excess keys are revoked at the vendor, each through
   the existing per-key deletion flow, before the role is written.
4. On IAM regions a demotion out of an unscoped role rewrites the statements
   naming that member on every bucket in the region, before the console row, and
   the response reports the buckets it reached. Demotion to a role that cannot
   hold a key deletes the member's keys in both models; every other narrowing
   follows live, since a key carries only what the policies give its member.
...
skipped and named in the acceptance event. An Owner or Admin invitation names no
buckets, and acceptance adds them to every policy in the region.
...
**Org deletion** needs no new path. Deleting the tenant destroys its principals,
...
names, evaluated per request by two membership tests. There is no principal,
user, role, or policy object anywhere, and no route updates a key.

At key creation Hilt stamps tenant-to-key delegations, one per command and
subject. Per request it re-delegates each command to the gateway with the bucket
as subject, expiring at the next UTC midnight plus clock skew; the gateway
caches those chains to the same horizon, and bucket-level operations are never
served from that cache. On key delete and bucket delete Hilt publishes
revocations before deleting its own rows and fails the call if the revocation
service errors, and the gateway's consumer drops the revoked key's cache. That
is the one existing path for pushing a change to a warm key.
...
2. Role-narrowing revocation with its preview route, dialogs, email, and audit
   shape. Merged. It revokes nothing by itself; a key is revoked only by a later
   narrowing that leaves it above the new role.
...
bucket, and it is not a permission ceiling, so statements would still decide the
permission set. It is a second input to an access decision, held outside the
policy and needing to be kept in step with the role. Holding the storage system
to exactly one input is worth the fan-out.
...
1. How long does a narrowing take to reach the gateway? The path is a revocation
   on the firehose rather than a cache expiry, so the number should be small,
   but nobody has measured it. Forge owns publishing that figure.
...
- [`2026-08-organizations-roles-m1.md`](./2026-08-organizations-roles-m1.md) for
  roles, the permission registry, the creator cap, the audit write path, and the
  script-only-PR rule.
...
- Hilt and Ingot behavior is read from `fil-forge/hilt` and `fil-forge/ingot` at
  `origin/main`.
