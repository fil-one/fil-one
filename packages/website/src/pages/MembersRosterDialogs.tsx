...
import { RoleNarrowingDialog } from '../components/RoleNarrowingDialog';
...
/**
 * Every confirmation `MembersRoster` asks before it acts, split into its own
 * module so the roster page is the mutations and the table, not also five
 * dialogs' worth of copy and preview-fetching.
 */
...
export interface DialogTargets {
  ...
  /** A change to someone else's row that takes a permission away. */
 ...
 *
 * Four of the five live here; the transfer's target is owned above, because a
...
function RoleNarrowingPrompt({
  ...
  // Shown in the dialog: tagged with the change it answered, because this
  // component stays mounted once opened — so it survives the closing fade but
  // not a dialog reopened about somebody else.
