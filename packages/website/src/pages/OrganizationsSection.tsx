...
import { cancelGuardedWork } from '../lib/use-warn-before-unload.js';
      ...
      : [
          {
            ...
            onSelect: () =>
              switchToOrg(membership.orgId, 'dashboard', {
...
export function OrganizationsSection({ me }: { me: MeResponse }) {
  ...
  const leave = useMutation({
    ...
    // Pinned to the org being left, so a switch in the meantime cannot turn it
    // into leaving a different one.
    mutationFn: (membership: OrgMembershipSummary) => removeMember(me.userId!, membership.orgId),
    onSuccess: (_result, membership) => {
      toast.success(`You left ${membership.orgName}`);
      // Anything still running (an upload) belongs to the org just left and
      // can only fail now, so it stops without asking. Asking would also let
      // the user stay in an org they no longer belong to.
      cancelGuardedWork();
      // The tab is still on the org just left, and so may the account's
      // default be, so a fresh `/me` alone would resolve back into it and land
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          try {
            await leave.mutateAsync(leaveTarget!);
          } catch {
