...
/** A role below Member, so a change off of it is a widening rather than a narrowing. */
const READ_ONLY: MemberSummary = {
  userId: 'user-4',
...
describe('MembersPage', () => {
  ...
  it('leaves the role picker usable while its own change is in flight', async () => {
    ...
    // A widening (ReadOnly → Member) applies on the spot, unlike a narrowing,
    // which the confirmation dialog would intercept before it ever mutates.
    renderPage(OrgRole.Owner, [OWNER, READ_ONLY, PLAIN]);

    const select = await screen.findByLabelText('Role for olive@example.com');
  ...
  it('tracks each in-flight row on its own account', async () => {
    const finish = heldCalls(mockUpdateRole);
    // Both widenings (ReadOnly → Member, Member → Admin) apply on the spot.
    renderPage(OrgRole.Owner, [OWNER, READ_ONLY, PLAIN]);

    fireEvent.change(await screen.findByLabelText('Role for olive@example.com'), {
      target: { value: OrgRole.Member },
    });
    await waitFor(() =>
      expect(screen.getAllByTestId('member-row')[1]).toHaveAttribute('aria-busy', 'true'),
