...
const me = (over: Partial<MeResponse>): MeResponse =>
  ({ orgId: 'org-1', orgName: 'Acme', email: 'a@b.co', ...over }) as unknown as MeResponse;
...
describe('the /create-organization route', () => {
  ...
  describe('beforeLoad', () => {
    it.each([
      ['already named', true],
      ['a pre-flag org with no value', undefined],
    ])(
      'sends a caller who arrives %s straight to their dashboard',
      async (_label, nameConfirmed) => {
        mockGetMe.mockResolvedValue(me({ nameConfirmed }));
  ...
  describe('the naming step', () => {
    beforeEach(() => {
      // The page reads `/me` for its suggested name and target org id; `beforeLoad`
    ...
    it('sends the freshly-named account to get-started, never the dashboard', async () => {
      ...
      await waitFor(() => expect(mockNavigate).toHaveBeenCalledWith({ to: '/get-started' }));
