...
// The support link routes through the router, which this test does not stand up.
vi.mock('./BaseLink', () => ({
...
describe('BillingRequiredGate', () => {
  ...
  it('points to support, with no card button, when billing cannot be read', async () => {
    mockGetBilling.mockRejectedValue(
      new Error('Unable to load billing details for this account. Please try again shortly.'),
    );

    renderGate(OrgRole.Owner);

    expect(await screen.findByText('Billing details are unavailable')).toBeInTheDocument();
  ...
  // The server answers billing only to billing.view; a 403 there is not an
  // outage, and must not replace the guidance with "unavailable".
  it.each([OrgRole.Member, OrgRole.ReadOnly])(
    'never reads billing for a %s, and keeps the guidance',
    async (role) => {
      mockGetBilling.mockRejectedValue(new Error('Forbidden'));

      renderGate(role);

      expect(await screen.findByText(/Ask an Owner to add a payment method/)).toBeInTheDocument();
      expect(screen.queryByText('Billing details are unavailable')).not.toBeInTheDocument();
