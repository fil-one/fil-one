...
export async function createAuroraAccessKey({
  ...
  assert(
    !!accessKeyId,
    `Aurora Portal API returned empty access key "accessKeyId" for tenant ${tenantId}`,
  );
  assert(
    !!accessKeySecret,
    `Aurora Portal API returned empty access key "accessKeySecret" for tenant ${tenantId}`,
  );
  assert(
    !!createdAt,
