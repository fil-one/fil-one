 ...
 * Stripe replay the billing write, and retries on a budget that fits the route's
 * 10s timeout. The `hubspot-contact-sync` cron repairs dropped writes; it calls
 * the client directly so failures surface to its counters.
 */
