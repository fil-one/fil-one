...
function buildClient(opts?: { fetch?: typeof fetch; token?: string }): FthClient {
  return createFthClient({
...
describe('FthClient endpoint coverage', () => {
  ...
  it('rotateToken POSTs to /tokens/rotate with no body and no Idempotency-Key', async () => {
    fetchMock = mockFetch(200, { token: 'new.token' });
    client = buildClient({ fetch: fetchMock });
    ...
    const req = lastRequest(fetchMock);
    expect(req.method).toBe('POST');
    expect(req.url).toBe('https://api.fortilyx.com/management/v1/tokens/rotate');
