...
describe('activate-subscription handler', () => {
  ...
  it('returns 410 ACCOUNT_DELETED before touching Stripe when the record is already mid-deletion', async () => {
    ...
    ddbMock.on(UpdateItemCommand).resolves({});
    // Wired so the *pre-fix* handler would have sailed all the way through
    // `trial_end: 'now'` — the failure below is the missing pre-check, not a
    // crash on an unstubbed mock.
    mockSubscriptionsUpdate.mockResolvedValue(mockSubscriptionResponse({ status: 'active' }));
    mockSubscriptionsCreate.mockResolvedValue(mockSubscriptionResponse({ status: 'active' }));
    mockSubscriptionsCancel.mockResolvedValue({ id: 'sub_test_456', status: 'canceled' });
    ...
    // The property that matters: no Stripe mutation runs at all, so no invoice
    // is generated by `trial_end: 'now'` and there is nothing to refund. The
    // write-time DELETION_GUARD fires too late to prevent the charge.
    expect(mockSubscriptionsUpdate).not.toHaveBeenCalled();
    expect(mockSubscriptionsCreate).not.toHaveBeenCalled();
    expect(mockSetupIntentsList).not.toHaveBeenCalled();
    expect(mockPromotionCodesList).not.toHaveBeenCalled();
    // Nothing to compensate for, and no billing write attempted.
    expect(mockSubscriptionsCancel).not.toHaveBeenCalled();
    expect(ddbMock.commandCalls(UpdateItemCommand)).toHaveLength(0);
    expect(mockSyncTenantStatusInProvisionedRegions).not.toHaveBeenCalled();

    // The pre-check is only worth having if it cannot read a stale record:
    // `deletionRequestedAt` is written synchronously before the confirm handler
    // answers 200, so an eventually-consistent read inside the replication
    // window would let a duplicate submit through to an unrefundable charge.
  ...
  it('returns 410 ACCOUNT_DELETED, not 400, when a mid-deletion record has no stripeCustomerId', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    // Webhook-born and legacy records exist without a stripeCustomerId. Such an
    // account must be told its account is gone, not handed the actionable
    // "set up a payment method first".
    ddbMock.on(GetItemCommand).resolvesOnce({
      Item: buildBillingRecord({
