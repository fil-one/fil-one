...
| `prefill-trial-entitlement-org.ts` | Stamp the org a trial went to on each pre-existing `TRIAL_ENTITLEMENT` claim; dry run by default, `--execute` writes. Run before deploying PR 717 (multi-org part 4)   |
...
| `trial-entitlement-org.ts` | Which org a pre-existing trial claim was spent on, for `prefill-trial-entitlement-org.ts`  |
