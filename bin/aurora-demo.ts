...
console.log('\n=== Listing access keys ===');
const { data: listData } = await listS3AccessKeys({
  client,
  path: { tenantId },
  throwOnError: false,
});
...
const { data: getById, response: getByIdResp } = await getS3AccessKey({
  client,
...
const { data: getByS3, response: getByS3Resp } = await getS3AccessKey({
  client,
